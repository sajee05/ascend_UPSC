...
export class DatabaseStorage implements IStorage {
  ...
  async getOverallAnalytics(): Promise<{
    ...
    const allTags = await db.select().from(tags).where(eq(tags.isAIGenerated, false));
    const subjectTags = [...new Set(allTags.map(t => t.tagName))];
    ...
    for (const subject of subjectTags) {
      ...
      const subjectQuestionIds = questionTags.map(t => t.questionId);
